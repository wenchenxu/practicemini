...
Page({
  ...
  onLoad(options) {
    const run = () => {
      ...
      this.setData({ id, mode, cityCode, city, selectedBranchCode, selectedBranchName, visibleFields: this.data.visibleFields || [] });
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        setTimeout(() => {
          ...
          wx.reLaunch({
            url: `/pages/contract/contract-list/index?cityCode=${encodeURIComponent(cityCode || '')}&city=${encodeURIComponent(city || '')}`
          });
        }, 1500);
      }
  ...
  toEdit() {
    ...
    wx.navigateTo({
      url: `/pages/contract/contract-new/index?city=${encodeURIComponent(city)}&mode=edit&id=${id}`
    });
  },
    ...
    try {
      ...
      this.setData({
        ...
        vehiclePickerRange: data.map(v => {
          let label = `${v.plate || ''} ${v.model || ''}`.trim();
          // display branchCode for debugging
          // if (v.branchCode) label += ` [${v.branchCode}]`;
          return label;
        }),
        vehiclePickerIndex: -1
      });
