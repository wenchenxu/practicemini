...
Page({
  ...
  goCity(e) {
    ...
    if (branches && branches.length > 0) {
      // 有分公司-> 跳转到分公司选择页
      wx.navigateTo({
        url: `/pages/core/branch-select/index?cityCode=${encodeURIComponent(code)}&city=${encodeURIComponent(name)}`
      });
    } else {
      // 无分公司-> 直接进入原本的城市门店页
      wx.navigateTo({
        url: `/pages/core/city/index?cityCode=${encodeURIComponent(code)}&city=${encodeURIComponent(name)}`
      });
    }
  },
  ...
  goFadadaTest() {
    wx.navigateTo({
      url: '/pages/admin/fadada-test/index'
    });
  },
  ...
  goVehicles() {
    wx.navigateTo({ url: '/pages/vehicle/vehicle-kanban/index' });
  },

  goDevTools() {
    wx.navigateTo({
      url: '/pages/admin/dev-tools/index'
    });
  }
});
